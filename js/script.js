document.addEventListener('DOMContentLoaded', () => {
    ...
    function populateDropdowns(bandsData, eventsData) {
        ...
        const bandNames = new Set(); const genres = new Set(); const countries = new Set(); const continents = new Set(); const eventNames = new Set(); const placeNames = new Set();
        bandsData.forEach(band => { if (band.band) bandNames.add(band.band); if (band.genres) band.genres.forEach(g => genres.add(g)); if (band.countries) band.countries.forEach(c => countries.add(c.country)); if (band.continents) band.continents.forEach(c => continents.add(c)); if (band.events_attended) { band.events_attended.forEach(e => { if (e.event) eventNames.add(e.event); if (e.place) placeNames.add(e.place); }); } });
        eventsData.forEach(event => { if (event.event) eventNames.add(event.event); if (event.place) placeNames.add(event.place); });
        // *** Usando locale 'de' para ordenar ***
        const fillSelect = (selectElement, optionsSet, defaultText) => { if (!selectElement) return; while (selectElement.options.length > 1) { selectElement.remove(1); } const sortedOptions = Array.from(optionsSet).sort((a, b) => a.localeCompare(b, 'de', { sensitivity: 'base' })); sortedOptions.forEach(optionText => { const option = document.createElement('option'); option.value = optionText; option.textContent = optionText; selectElement.appendChild(option); }); };
    ...
    function displayBandResults(bands) {
        ...
        if (!bands || bands.length === 0) {
         ...
         bands.forEach(band => {
            ...
            let likeIndicator = ''; if (band.likes) { likeIndicator = `<span class="like-indicator like-${band.likes}" title="${getLikeText(band.likes)}"></span>`; } // getLikeText traducirá el title
             // *** Etiquetas en alemán ***
            const genresHtml = band.genres ? band.genres.join(', ') : '<em>k.A.</em>';
            let countriesHtml = '<em>k.A.</em>'; if (band.countries && band.countries.length > 0) { countriesHtml = band.countries.map(c => { const flagHtml = c.flag ? `<img src="img/flags/${c.flag.toLowerCase()}.png" alt="${c.flag}" class="flag-icon" onerror="this.style.display='none'">` : ''; return `${c.country || '?'}${flagHtml}`; }).join(', '); }
            const continentsHtml = band.continents ? band.continents.join(', ') : '<em>k.A.</em>';
            let eventsHtml = '<em>Keine</em>'; if (band.events_attended && band.events_attended.length > 0) { eventsHtml = `<ul>${band.events_attended.map(e => `<li>${e.event || '?'} (${e.place || '?' }, ${e.date || '?'})</li>`).join('')}</ul>`; }
            card.innerHTML = `
